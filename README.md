...
1.  <b>My application is using a GET request to display data from OpenFDA through its [Drug API Endpoints](https://open.fda.gov/apis/drug/ "OpenFDA Drug API Endpoints").</b>
  2. <b>I use a dropdown menu that allows the provider to select a drug from the OpenFDA Drug API. I populated the drug dropdown with a list of the most common medications seen on the NCLEX-RN exam for registered nurse licensure in the USA, Canada, and Australia.</b>
